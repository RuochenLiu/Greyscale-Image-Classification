...
Feed the baseline training model with SIFT data. 
```{r}
tm_test_bs <- system.time(pred_test.bs <- Test(baseline, feature_test))
save(pred_test.bs, file="../output/pred_test_bs.RData")
```

####advanced model

Feed the advanced training model with HOG data.
```{r}
tm_test_ad <- system.time(pred_test.ad <- Test(advanced, feature_test))
save(pred_test.ad, file="../output/pred_test_ad.RData")
``` 
